...
Here are three primary enterprise use cases enabled by Phase 127:
1. **Predicting lateral**: Predicting lateral movement paths in a multi-tenant cloud environment.
2. **Identifying chokepoints**: Identifying chokepoints in the enterprise trust graph to prioritize remediation.
3. **Simulating breach**: Simulating breach propagation scenarios to quantify financial risk exposure.
...
|---|---|---|
| `path_id` | `str` | Internal persistent representation of path_id. |
| `source_node` | `str` | Internal persistent representation of source_node. |
| `target_node` | `str` | Internal persistent representation of target_node. |
| `complexity` | `float` | Internal persistent representation of complexity. |
...
| Field Name | Type | Description |
|---|---|---|
| `movement_id` | `str` | Internal persistent representation of movement_id. |
| `source_host` | `str` | Internal persistent representation of source_host. |
| `dest_host` | `str` | Internal persistent representation of dest_host. |
| `credential_used` | `str` | Internal persistent representation of credential_used. |
| `probability` | `float` | Internal persistent representation of probability. |

### Model: `AssetExposure`
| Field Name | Type | Description |
|---|---|---|
| `exposure_id` | `str` | Internal persistent representation of exposure_id. |
| `asset_id` | `str` | Internal persistent representation of asset_id. |
| `exposure_level` | `str` | Internal persistent representation of exposure_level. |
| `vulnerabilities` | `List[str]` | Internal persistent representation of vulnerabilities. |
| `score` | `float` | Internal persistent representation of score. |

### Model: `BreachScenario`
| Field Name | Type | Description |
|---|---|---|
| `scenario_id` | `str` | Internal persistent representation of scenario_id. |
| `name` | `str` | Internal persistent representation of name. |
| `entry_point` | `str` | Internal persistent representation of entry_point. |
| `target_asset` | `str` | Internal persistent representation of target_asset. |
| `estimated_impact` | `float` | Internal persistent representation of estimated_impact. |


## REST APIs
The following endpoints are exposed by this module:
- `GET /api/v1/phase127/status`: Retrieves the operational health, uptime, and metadata of this module.
- `POST /api/v1/phase127/execute`: Executes the core business logic or orchestrator function for a specific tenant.
...
The module will be created at `src/phase_127_enterprise_attack_path_intelligence/` with the following files:
- `src/phase_127_enterprise_attack_path_intelligence/__init__.py`
- `src/phase_127_enterprise_attack_path_intelligence/models.py`
- `src/phase_127_enterprise_attack_path_intelligence/schemas.py`
- `src/phase_127_enterprise_attack_path_intelligence/store.py`
- `src/phase_127_enterprise_attack_path_intelligence/service.py`
- `src/phase_127_enterprise_attack_path_intelligence/analytics.py`
- `src/phase_127_enterprise_attack_path_intelligence/api.py`

## GitHub Issue Title
`feat(phase-127): implement enterprise attack path intelligence platform`

## GitHub Issue Description
```markdown
## Objective
Implement Phase 127: Enterprise Attack Path Intelligence Platform under the AegisGraph Sentinel X enterprise platform.

## Proposed Components
1. Models layer for AttackPath, LateralMovement, AssetExposure, BreachScenario
2. Thread-safe store layer with full lock protection
3. Service orchestrator providing analyze_attack_path, detect_lateral_movement, assess_exposure, simulate_breach
4. Analytics engine for dashboard KPI reporting
5. FastAPI router at /api/v1/phase127
...
## Pull Request Title
`feat(scope): implement phase 127 - enterprise attack path intelligence platform`
...
### Overview
This PR delivers the complete, enterprise-grade implementation of Phase 127: Enterprise Attack Path Intelligence Platform.

### Files Added
- `src/phase_127_enterprise_attack_path_intelligence/` - Complete module code
- `tests/test_phase_127_enterprise_attack_path_intelligence.py` - Verification tests
...
```markdown
The implementation of Phase 127 is ready for review. 
