...
Here are three primary enterprise use cases enabled by Phase 144:
1. **Screening entities**: Screening entities against OFAC and global sanctions registries in under 50ms.
2. **Detecting micro-structuring**: Detecting micro-structuring transaction flows designed to bypass standard reporting rules.
3. **Auto-compiling Suspicious**: Auto-compiling Suspicious Activity Reports (SAR) for submission to regulatory networks.
...
|---|---|---|
| `list_id` | `str` | Internal persistent representation of list_id. |
| `name` | `str` | Internal persistent representation of name. |
| `entities_count` | `int` | Internal persistent representation of entities_count. |
| `last_update` | `str` | Internal persistent representation of last_update. |

### Model: `AmlAlert`
| Field Name | Type | Description |
|---|---|---|
| `alert_id` | `str` | Internal persistent representation of alert_id. |
| `account_id` | `str` | Internal persistent representation of account_id. |
| `alert_type` | `str` | Internal persistent representation of alert_type. |
...
| `entity_name` | `str` | Internal persistent representation of entity_name. |
| `list_name` | `str` | Internal persistent representation of list_name. |
...
| `account_id` | `str` | Internal persistent representation of account_id. |
| `stage` | `str` | Internal persistent representation of stage. |
| `assigned_to` | `str` | Internal persistent representation of assigned_to. |


## REST APIs
The following endpoints are exposed by this module:
- `GET /api/v1/phase144/status`: Retrieves the operational health, uptime, and metadata of this module.
- `POST /api/v1/phase144/execute`: Executes the core business logic or orchestrator function for a specific tenant.
...
The module will be created at `src/phase_144_global_aml_intelligence_platform/` with the following files:
- `src/phase_144_global_aml_intelligence_platform/__init__.py`
- `src/phase_144_global_aml_intelligence_platform/models.py`
- `src/phase_144_global_aml_intelligence_platform/schemas.py`
- `src/phase_144_global_aml_intelligence_platform/store.py`
- `src/phase_144_global_aml_intelligence_platform/service.py`
- `src/phase_144_global_aml_intelligence_platform/analytics.py`
- `src/phase_144_global_aml_intelligence_platform/api.py`

## GitHub Issue Title
`feat(phase-144): implement global aml intelligence platform`

## GitHub Issue Description
```markdown
## Objective
Implement Phase 144: Global AML Intelligence Platform under the AegisGraph Sentinel X enterprise platform.

## Proposed Components
1. Models layer for SanctionsList, AmlAlert, SanctionsMatch, AmlCase
2. Thread-safe store layer with full lock protection
3. Service orchestrator providing screen_sanctions, process_aml_alert, create_aml_case, update_sanctions_list
4. Analytics engine for dashboard KPI reporting
5. FastAPI router at /api/v1/phase144
...
## Pull Request Title
`feat(scope): implement phase 144 - global aml intelligence platform`
...
### Overview
This PR delivers the complete, enterprise-grade implementation of Phase 144: Global AML Intelligence Platform.

### Files Added
- `src/phase_144_global_aml_intelligence_platform/` - Complete module code
- `tests/test_phase_144_global_aml_intelligence_platform.py` - Verification tests
...
```markdown
The implementation of Phase 144 is ready for review. 
