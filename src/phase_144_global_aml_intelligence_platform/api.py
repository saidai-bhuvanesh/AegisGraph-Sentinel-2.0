"""
FastAPI routing endpoints for Global AML Intelligence Platform
"""
...
router = APIRouter(prefix="/api/v1/phase144", tags=["Phase144"])
...
async def get_status(api_key: str = Depends(verify_rbac)):
    return {"status": "healthy", "phase": 144, "name": "Global AML Intelligence Platform"}
...
async def execute_module(request: ExecutionRequest, api_key: str = Depends(verify_rbac)):
    ...
    return ExecutionResponse(status="success", phase=144, result=res)
